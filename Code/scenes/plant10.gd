...
func _process(delta):
	...
	if toPercent(clickCount) >= 10.0 && toPercent(clickCount) <= 30.0:
		self.texture = ResourceLoader.load("res://planten/part10/fase_2.png")
	elif toPercent(clickCount) >= 30.0 && toPercent(clickCount) <= 60.0:
		self.texture = ResourceLoader.load("res://planten/part10/fase_3.png")
	elif toPercent(clickCount) >= 60.0 && toPercent(clickCount) < 100.0:
		self.texture = ResourceLoader.load("res://planten/part10/fase_4.png")
	elif toPercent(clickCount) >= 100.0:
		self.texture = ResourceLoader.load("res://planten/part10/fase_5.png")
